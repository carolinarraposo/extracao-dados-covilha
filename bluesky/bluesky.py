...
client = Client()
handle = 'patriciamarcos.bsky.social'
password = 'foui-32jj-f4wm-73ci'
...
with open('bluesky_posts.csv', 'w', newline='', encoding='utf-8') as f:
    ...
    for p in dados_finais:
        ...
        txt_comentarios = " | ".join([f"[@{c['autor_comentario']}: {c['texto_comentario']} (Likes: {c['likes_comentario']})]" for c in p['comentarios']])
